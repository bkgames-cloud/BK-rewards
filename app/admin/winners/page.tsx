  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Gagnants en attente d&apos;envoi (carte) ou déjà crédités.</p>
      </div>
      <Button onClick={() => void testEmail()} disabled={testingEmail} className="w-full sm:w-auto">
      ...
      {loading ? (
          ...
          {rewards.map((reward) => (
            <Card key={reward.id} className="border border-border/50 bg-[#1a1a1a] shadow-lg">
              <CardHeader className="pb-2">
                <CardTitle className="text-base text-foreground">{rewardLabel(reward.reward_type)}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-2 text-sm text-muted-foreground">
                <p>
                  Gagnant : <span className="font-semibold text-foreground">{reward.display_name}</span>
                </p>
                <p>
                  Email : <span className="font-semibold text-foreground">{reward.email}</span>
                </p>
                <p>
                  Statut : <span className="font-semibold text-foreground">{reward.status}</span>
                </p>
                <p>Créé le : {new Date(reward.created_at).toLocaleDateString("fr-FR")}</p>
                {reward.status === "pending" && (
                  <Button className="w-full" onClick={() => void markSent(reward.id)} disabled={updatingId === reward.id}>
        ...
        <Card className="border border-border/50 bg-[#1a1a1a] shadow-lg">
          <CardContent className="py-10 text-center text-sm text-muted-foreground">Aucun gain en attente.</CardContent>
        </Card>
      )}
    </div>
  )
