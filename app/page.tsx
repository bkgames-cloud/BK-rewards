...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const fetchRecentWinners = async () => {
      ...
      const primary = await supabase.from("winners").select("*").order("created_at", { ascending: false }).limit(5)

      let rows: Record<string, unknown>[] = []
      if (primary.error) {
        const fallback = await supabase.from("winners").select("*").order("id", { ascending: false }).limit(5)
        ...
        <div className="space-y-4 p-4">
          ...
          {showAccountDeletedBanner ? (
            <Card className="border border-green-500/40 bg-green-950/30 shadow-lg">
              <CardContent className="flex flex-col gap-2 p-4 text-sm text-green-50/95 sm:flex-row sm:items-center sm:justify-between">
                <p>Votre compte a bien été supprimé. Merci d&apos;avoir utilisé BKG Rewards.</p>
                <Button
                  type="button"
                  variant="secondary"
                  size="sm"
          ...
          <Card className="border border-border/50 bg-[#1a1a1a] shadow-lg">
            ...
            <CardContent className="space-y-2">
              {winners.length === 0 ? (
                <p className="text-sm text-muted-foreground">Aucun gagnant récent pour le moment.</p>
              ) : (
          ...
          <Card className="border border-border/50 bg-[#1a1a1a] shadow-lg">
            ...
            <CardContent className="space-y-2 text-sm text-muted-foreground">
              <p>Accédez à la roue, au scratch et au Tap-Tap sur la page Concours.</p>
              <Button
                asChild
