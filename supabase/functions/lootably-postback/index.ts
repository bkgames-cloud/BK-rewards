...
Deno.serve(async (req) => {
  try {
    ...
    const LOOTABLY_POSTBACK_SECRET = (Deno.env.get("LOOTABLY_POSTBACK_SECRET") ?? "").trim()
    if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) return json({ ok: false, error: "missing_supabase_env" }, 500)
    if (!LOOTABLY_POSTBACK_SECRET) return json({ ok: false, error: "missing_postback_secret" }, 503)

    const url = new URL(req.url)
    const secret = getParam(url, ["secret", "key", "token"])
    if (secret !== LOOTABLY_POSTBACK_SECRET) return json({ ok: false, error: "unauthorized" }, 401)
